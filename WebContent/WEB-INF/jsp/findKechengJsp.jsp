...
<link rel="stylesheet" href="${pageContext.request.contextPath }/lib/bootstrap/css/bootstrap.css" />
  ...
  <div class="container-fluid">
		    ...
		    <div class="row">
		        <div class="col-md-2">
		            <div class="list-group">
		                <a href="${pageContext.request.contextPath}/banji/findKecheng.action" class="list-group-item active">课程管理</a>
		                <a href="${pageContext.request.contextPath}/banji/addKecheng.action" class="list-group-item">课程添加</a>
		            </div>
		        </div>
		        <div class="col-md-10">
				...
				<c:forEach items="${requestScope.list.list}" var="kecheng">
					<tr>
						<td>${kecheng['id']}</td>
						<td>${kecheng['name'] }</td>
						<td>${kecheng['credit'] }</td>
						<td><a href="javascript:delStudent(${kecheng['id'] });">删除</a></td>
						<td><a href="${pageContext.request.contextPath }/banji/updateKecheng.action?id=${kecheng['id'] }">修改</a></td>
					</tr>
				</c:forEach>
		                </tbody>
		            </table>
		       ...
		       <!-- 分页开始 -->
			    ...
			    <c:if test="${list.pageIndex==1}">
		           ...
		           <c:if test="${list.pageIndex!=1}">
		              <li>
		                 <a href="${pageContext.request.contextPath}/banji/findKecheng.action?pageIndex=${list.pageIndex-1}" aria-label="Previous">
		                   <span aria-hidden="true">&laquo;</span>
		                 </a>
		              </li>
		           </c:if>
              ...
              <c:if test="${list.pageIndex!=page}">
                   <li><a href="${pageContext.request.contextPath}/banji/findKecheng.action?pageIndex=${page}">${page}</a></li>
              </c:if>
	...
	function delStudent(id) {
	       ...
	       if (isDel) {
	           //要删除
	           location.href = "${pageContext.request.contextPath}/banji/deleteKecheng.action?id="
	                  + id;
	       }
	    }
